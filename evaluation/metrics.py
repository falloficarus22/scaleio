...
def calculate_psr(img1: torch.Tensor, img2: torch.Tensor, max_val: float = 1.0) -> float:
    if img1.dim() == 3:
...
def calculate_ssim(img1: torch.Tensor, img2: torch.Tensor, window_size: int = 11, max_val:float = 1.0) -> float:
    ...
    mu1 = F.conv2d(img2, window, padding=window_size//2, groups=img1.size(1))
...
def tensor_to_numpy(img: torch.Tensor) -> np.ndarray:
    ...
    return (img * 255).astype(np.unit8)
