...
class RealESRGANUpscaler:
    ...
    def upscale_image(self, input_path, output_path=None):
        ...
        if output_path is None:
            output_path = input_path.parent / f"{input_path.stem}_upscaled_{self.scale_factor}x{input_path.suffix}"
        else:
    ...
    def upscale_batch(self, input_dir, output_dir=None, file_extensions=('.jpg', '.jpeg', '.png', '.bmp', '.tiff')):
        """
        ...
        for img_file in tqdm(image_files, desc="Upscaling images"):
            output_path = output_dir / f"{img_file.stem}_upscaled_{self.scale_factor}x{img_file.suffix}"
            try:
...
def main():
    parser = argparse.ArgumentParser(description="Local Image Upscaler using RealESRGAN")
    ...
    parser.add_argument('-s', '--scale', type=int, default=4, choices=[2, 4, 8],
    ...
    if args.scale != int(args.model.split('x')[1]):
        print(f"Warning: Scale factor {args.scale} doesn't match model {args.model}")
        print(f"Using scale factor from model: {int(args.model.split('x')[1])}")
        args.scale = int(args.model.split('x')[1])
